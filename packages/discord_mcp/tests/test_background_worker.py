...
from discord_mcp.db import get_discord_connection, get_message_count, get_earliest_timestamp_from_db, get_latest_timestamp_from_db
...
class TestableDiscordClient(MockDiscordClient):
    ...
    def get_messages_since(self, channel_id: str, since: datetime, until=None, before=None):
    ...
    def get_guild_channels(self, guild_id: str):
        ...
        # Return only first channel for testing
        if all_channels:
            yield all_channels[0]
...
class TestBackgroundWorker:
    ...
    def setup_method(self):
        ...
        self.db_path_patcher = patch('discord_mcp.db.DISCORD_MCP_DB_PATH', Path(self.temp_db.name))
    ...
    def test_time_window_logic_empty_db(self):
        ...
        assert abs((start_time - expected_start).total_seconds()) < 60  # Within 1 minute of expected
    ...
    def test_background_worker_single_run_two_batches(self):
        ...
        with get_discord_connection() as conn:
            ...
            assert 3 <= latest_days_old <= 5, f"Latest message should be ~4 days old, got {latest_days_old}"
            assert 6 <= earliest_days_old <= 8, f"Earliest message should be ~7 days old, got {earliest_days_old}"
            ...
            assert second_latest_dt > latest_dt, "Latest timestamp should move forward (newer messages)"
            ...
            assert 0 <= latest_days_old <= 2, f"New latest message should be ~1 day old, got {latest_days_old}"
            
            print(f"✅ Time range expanded from {(now - latest_dt).days} to {latest_days_old} days old")
    ...
    def test_all_data_stored_correctly(self):
        ...
        with get_discord_connection() as conn:
            ...
            print(f"Stored: {message_count} messages, {user_count} users, {channel_count} channels, {guild_count} guilds")
...
class TestMockClientExtensions:
    ...
    def test_message_timestamp_adjustment(self):
        ...
        for message in messages:
            ...
            assert 3 <= days_old <= 8, f"Message should be 4-7 days old, got {days_old} days"