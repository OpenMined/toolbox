...
def get_time_windows_to_fetch(earliest_ts: Optional[str], latest_ts: Optional[str], days_back: int = 365) -> List[tuple[datetime, datetime]]:
    """
    ...
    if earliest_ts is None or latest_ts is None:
        ...
        # Make sure all datetimes have consistent timezone handling
        if earliest_dt.tzinfo is not None and now.tzinfo is None:
            now = now.replace(tzinfo=earliest_dt.tzinfo)
            min_time = min_time.replace(tzinfo=earliest_dt.tzinfo)
        elif earliest_dt.tzinfo is None and now.tzinfo is not None:
            earliest_dt = earliest_dt.replace(tzinfo=now.tzinfo)
...
def run_discord_mesage_dump_background_worker_single(conn, client: DiscordClient, guild_names: List[str] = None):
    """
    ...
    windows_to_fetch = get_time_windows_to_fetch(earliest_message_ts, latest_message_ts)
            ...
            for channel_info in channels:
                ...
                for start_time, end_time in windows_to_fetch:
                    ...
                    try:
                        ...
                        for message_data in client.get_messages_since(channel_id, start_time, until=end_time):
                            message_count += 1
                            ...
                            message_model = DiscordMessage.from_discord_api(message_data)
                        ...
                        if message_count > 0:
                            print(f"    Added {message_count} messages and {len(user_cache)} users")
...
def run_discord_mesage_dump_background_worker_loop(guild_names: List[str] = None, sleep_interval: int = 3600):
    """
    Run the Discord message dump background worker in a continuous loop.
    
    Args:
        guild_names: List of guild names to process, or None for all guilds
        sleep_interval: Time to sleep between runs in seconds (default: 1 hour)
    """
    print(f"Starting Discord message dump background worker loop (sleep interval: {sleep_interval}s)")
    ...
    while True:
        try:
            with get_discord_connection() as conn:
                ...
                try:
                    run_discord_mesage_dump_background_worker_single(conn, client, guild_names)
                finally:
                    client.close()
