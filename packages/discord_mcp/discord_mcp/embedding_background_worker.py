...
def get_embedding_ollama(query: str) -> List[float]:
    ...
    response = requests.post(f"{settings.ollama_url}/api/embeddings", json=payload, timeout=10)
...
def run_embedding_background_worker_single(conn):
    ...
    if not check_rate_limit(conn):
        print(f"Rate limit reached ({settings.max_messages_per_hour} messages/hour), skipping this iteration")
        return
...
def start_embedding_background_worker() -> threading.Thread:
    """Start the embedding background worker in a separate thread"""
    stop_event = threading.Event()
    thread = threading.Thread(
        target=run_embedding_background_worker_loop,
        args=(stop_event,),
        daemon=True
    )
