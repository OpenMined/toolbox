// API client for handling data fetching with mock data support
const MOCK_DATA = import.meta.env.VITE_MOCK_DATA === "true"; // Default to false (use backend)
...
class APIClient {
  ...
  getMockData(endpoint, options) {
    ...
    switch (endpoint) {
      case "/data-sources":
        return Promise.resolve(this.mockDataSources());
      case "/data-collections":
        return Promise.resolve(this.mockDataCollections());
      case "/smart-lists":
        if (options.method === "POST") {
          // Handle smart list creation
          return Promise.resolve(this.mockCreateSmartList(options.body));
        }
        return Promise.resolve(this.mockSmartLists());
      default:
        if (
          endpoint.startsWith("/smart-lists/") &&
          endpoint.endsWith("/items")
        ) {
          const listId = parseInt(endpoint.split("/")[2]);
          return Promise.resolve(this.mockSmartListItems(listId));
        }
        if (
          endpoint.startsWith("/smart-lists/") &&
          endpoint.endsWith("/summary")
        ) {
          const listId = parseInt(endpoint.split("/")[2]);
          return Promise.resolve(this.mockSummary(listId));
        }
        if (endpoint.startsWith("/chats/")) {
          const listId = parseInt(endpoint.split("/")[2]);
          return Promise.resolve(this.mockChats(listId));
        }
        return Promise.reject(
          new Error(`Mock endpoint not implemented: ${endpoint}`),
        );
    }
  }
    ...
    return [
      {
        ...
        icon: "twitter",
        description:
          "Connect your Twitter account to analyze tweets and conversations",
      ...
      },
      {
        ...
        icon: "discord",
        description:
          "Connect Discord servers to analyze messages and conversations",
    ...
    return [
      {
        id: "twitter",
        name: "Twitter",
        items: [], // Items would be fetched separately when needed
      },
      {
        id: "ai-papers",
        name: "AI Papers",
    ...
    return [
      {
        ...
        computedItems: [], // Would be fetched from backend
        itemCount: 23,
      },
      {
        id: 2,
  ...
  mockSmartListItems(listId) {
    // Return the existing mock tweet data based on list ID
    const mockData = {
      1: [
        {
          id: 1,
  ...
  mockCreateSmartList(body) {
    // Parse the body if it's a string
    const listData = typeof body === "string" ? JSON.parse(body) : body;

    // Create a new smart list with generated ID
    const newList = {
      id: Date.now(), // Use timestamp as ID
      name: listData.name || "Untitled List",
      listSources: listData.listSources || listData.sources || [],
      computedItems: [],
      itemCount: 0,
      ...listData,
    };
      ...
      1: [
        {
          ...
          messages: [
            ...
            },
            {
              ...
              role: "assistant",
              content:
                "Based on the tweets, developers favor lo-fi beats, synthwave, and atmospheric music that creates an immersive coding environment.",
              timestamp: new Date().toISOString(),
            },
          ],
        },
      ],
      // ... other lists would have their chats here
    };

    return mockChats[listId] || [];
  }
