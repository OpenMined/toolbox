...
class TestDownloadMessages:
    ...
    def test_download_messages_structure(self):
        ...
        assert len(messages) > 0, (
            "Messages list should not be empty - mock should return test data"
        )
        assert len(messages) >= 10, (
            f"Expected some test data, got only {len(messages)} messages"
        )
    ...
    def test_download_messages_with_users_structure(self):
        ...
        messages = result["messages"]
        users = [m["author"] for m in messages]
        ...
        assert len(users) > 0, (
            "Users list should not be empty - mock should return test data"
        )
        assert metadata["message_count"] == len(messages)
...
class TestDownloadChannels:
    ...
    def test_download_channels_structure(self):
        ...
        result = download_channels(token="mock_token", guild_name="Claude Developers")
        ...
        assert len(channels) > 0, (
            "Channels list should not be empty - mock should return test data"
        )
        assert len(channels) >= 10, (
            f"Expected substantial test data, got only {len(channels)} channels"
        )
...
class TestDownloadGuilds:
    ...
    def test_download_guilds_structure(self):
        ...
        assert len(guilds) > 0, (
            "Guilds list should not be empty - mock should return test data"
        )
...
class TestMockDiscordClient:
    ...
    def test_mock_client_get_user_guilds(self):
        ...
        assert isinstance(guilds, list)
        assert len(guilds) > 0, (
            "Mock client should return non-empty guilds list from test data"
        )
    ...
    def test_mock_client_get_messages(self):
        ...
        assert isinstance(messages, list)
        assert len(messages) > 0, (
            "Mock client should return non-empty messages list from test data"
        )
    ...
    def test_mock_client_get_guild_channels(self):
        ...
        assert isinstance(channels, list)
        assert len(channels) > 0, (
            "Mock client should return non-empty channels list from test data"
        )
