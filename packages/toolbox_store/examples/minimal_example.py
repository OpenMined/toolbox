...
query = (
    store.search_chunks()
    .semantic("Airport security", score_threshold=0.3)
    .where({"created_at__gte": datetime(2025, 9, 1)})
    .where({"created_at__lt": datetime(2025, 10, 1)})
    .keyword("history AI")
    .hybrid(semantic_weight=0.8, keyword_weight=0.2)
    .rerank()
    .chunk_limit(10)
)
...
for chunk in chunks:
    print(f"- Chunk (doc {chunk.document_id}, score {chunk.score:.4f}):")
...
for doc in docs:
    ...
    print(f"  max chunk score: {doc.chunks[0].score if doc.chunks else 'N/A'}")
    print()
