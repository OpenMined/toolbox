...
from discord_mcp.embedding_background_worker import run_embedding_background_worker_single
from discord_mcp.mcp_server import search_messages
...
def test_embedding_background_worker():
    ...
    try:
        ...
        with patch('discord_mcp.embedding_background_worker.get_embedding', side_effect=mock_get_embedding):
            with get_discord_connection(tmp_db_path) as conn:
                ...
                messages_without_embeddings_after = get_messages_without_embeddings(conn)
                assert len(messages_without_embeddings_after) < 3  # Some messages should now have embeddings
