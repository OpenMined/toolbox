...
class TestPermissions:
    ...
    def test_permission_computation_with_mock_client(self):
        ...
        assert isinstance(results, dict), (
            "Should return a dictionary of channel_id -> bool"
        )
        assert len(results) > 0, "Should have at least some channel results"

        # Count accessible channels
        accessible_channels = [
            channel_id for channel_id, has_access in results.items() if has_access
        ]
        inaccessible_channels = [
            channel_id for channel_id, has_access in results.items() if not has_access
        ]
        ...
        assert len(accessible_channels) == 12, (
            f"Expected 12 accessible channels, got {len(accessible_channels)}"
        )
        ...
        assert len(results) == 47, (
            f"Expected 47 total text channels, got {len(results)}"
        )
    ...
    def test_accessible_channel_names(self):
        ...
        for expected_channel in expected_accessible:
            assert expected_channel in accessible_channel_names, (
                f"Expected {expected_channel} to be accessible"
            )
        ...
        assert len(unexpected_accessible) == 0, (
            f"Unexpected accessible channels: {unexpected_accessible}"
        )
    ...
    def test_permission_computation_consistency(self):
        ...
        assert accessible1 == accessible2 == 12, (
            "Both runs should have 12 accessible channels"
        )
    ...
    def test_mock_client_permission_methods(self):
        ...
        assert isinstance(guild_roles, dict), (
            "Should return dict of role_id -> role_data"
        )
        ...
        channels_with_overwrites = [
            c for c in channels if c.get("permission_overwrites")
        ]
        assert len(channels_with_overwrites) > 0, (
            "Should have channels with permission overwrites"
        )
