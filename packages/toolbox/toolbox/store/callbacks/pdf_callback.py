...
def start_ollama_and_pull_model():
    ...
    try:
        ...
        result = subprocess.run(["ollama", "pull", "nomic-embed-text:v1.5"], capture_output=True, text=True)
        return result.returncode == 0
...
Press Enter once you've installed these dependencies to continue.""")
        ...
        if ollama_installed() and not ollama_running():
            ...
            if start_ollama_and_pull_model():
                ...
                print("⚠️  Failed to start Ollama automatically. Please run 'ollama serve' and 'ollama pull nomic-embed-text' manually")
...
class InstallPDFMCPCallback(Callback):
    def on_run_mcp(self, context: "InstallationContext", *args, **kwargs):
        ...
        print("📄 PDF MCP installed! Document loading will start automatically once the server is running.")
        print("💡 Use 'get_loading_progress()' to track progress, or 'start_auto_loading()' to start manually.")
