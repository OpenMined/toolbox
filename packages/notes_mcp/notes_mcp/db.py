...
def insert_audio_chunk(conn, chunk_id: int, file_path: str, timestamp: datetime, user_email: str):
    cursor = conn.cursor()
    cursor.execute("""
...
def insert_transcription(conn, transcription_id: int, audio_chunk_id: int, offset_index: int, timestamp: datetime, transcription: str, device: str, is_input_device: bool, speaker_id: int, transcription_engine: str, start_time: float, end_time: float, text_length: int):
    cursor = conn.cursor()
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (transcription_id, audio_chunk_id, offset_index, timestamp, transcription, device, is_input_device, speaker_id, transcription_engine, start_time, end_time, text_length))
    conn.commit()
